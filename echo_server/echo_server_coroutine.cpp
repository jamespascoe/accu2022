...
// -Werror echo_server_coroutine.cpp -o coroutine -l pthread
//
...
boost::asio::awaitable<void> echo(tcp::socket peer_socket,
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      co_await async_write(peer_socket, buffer(buf, len), use_awaitable);
    }
...
int main() {
  try {
    ...
    co_spawn(ctx, echo(std::move(socket), std::move(acceptor)), detached);
